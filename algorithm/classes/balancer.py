...
class Balancer:
    ...
    def _set_orders(
        ...
        for station in carpark.active_charging_stations:
            if usage < energy_usage_demand * (
                1 + sign * margin
            ):  # we start with usage bigger than expected so we end balancing when usage is smaller than expected usage + margin
                demand_met = True
                break
    ...
    def _balance_case(
        self, carpark: Carpark, energy_usage_demand, orders_to_balance: tuple
    ):
        """
        ...
        while curr_order_index < balance_attempts and not demand_met:
            usage, demand_met = self._set_orders(
                carpark, energy_usage_demand, usage, orders_to_balance[curr_order_index]
            )
            curr_order_index += 1
        return usage
